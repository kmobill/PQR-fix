...
$("#dash1refresh").click(function(){
    ...
    if (kpifechainicio == "" || kpifechafin == ""){
        ...
        get_detail(datatab, "backend/dashboardkpiexperiencia.php?action=fcrdetailtodos", kpifechainicio, kpifechafin)

        urlkpinps = "backend/dashboardkpiexperiencia.php?action=npstodos"
        var datatab1 = $("#datatable-detail-nps")
        get_detail(datatab1, "backend/dashboardkpiexperiencia.php?action=npstodosdetail", kpifechainicio, kpifechafin)
    }
    else {
        urlkpifcr = "backend/dashboardkpiexperiencia.php?action=fcrfechas"
        var datatab = $("#datatable-detail-fcr-no")
        get_detail(datatab, "backend/dashboardkpiexperiencia.php?action=fcrdetailfechas", kpifechainicio, kpifechafin)

        urlkpinps = "backend/dashboardkpiexperiencia.php?action=npsfechas"
        var datatab1 = $("#datatable-detail-nps")
        get_detail(datatab1, "backend/dashboardkpiexperiencia.php?action=npsfechasdetail", kpifechainicio, kpifechafin)
    }
    $.ajax({
        url: urlkpifcr,
        data: {
...
function get_detail(datatab, urlaction, fechainicio, fechafin){
    ddetailfcrno = datatab.DataTable({
        ...
        ajax:{
            ...
            url: urlaction, 
            data: {
                fechainicio: (fechainicio == "" || fechainicio == null)?"1900-01-01":fechainicio,
                fechafin: (fechafin == "" || fechafin == null)?obtenerFecha():fechafin
            },            
        },
        columnDefs: [
            {title: "Comentario", targets: 0}
        ],
        order: [[ 0, "desc" ]],
    ...
    ddetailfcrno.buttons(1, null ).container().appendTo(
        ddetailfcrno.table().container()
    );
}
