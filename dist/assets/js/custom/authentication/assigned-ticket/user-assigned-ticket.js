...
$('#datatable-ajax tbody').on('click', 'td.details-control', function(){
	...
	vEditTRespuesta = dataglb[10] + " " + dataglb[16]
        ...
        var redsocial = (dataglb[18] == "" || dataglb[18] == null)?"No Aplica":dataglb[18]
        var agencia = (dataglb[19] == "" || dataglb[19] == null)?"No Aplica":dataglb[19]
        var comentario = dataglb[20]
        row.child(format(cedula, mail, phone, producto, tincidencia, stincidencia, area, trespuesta, creadop, aasignado, canal, redsocial, agencia, comentario)).show()
        tr.addClass('shown')
    }
})

/**************************************************************************En proceso*/
$('#datatable-ajax-proceso tbody').on('click', 'td.details-control', function(){
    var tr = $(this).closest('tr')
    ...
    if (row.child.isShown()){
        ...
        var redsocial = (dataglb[18] == "" || dataglb[18] == null)?"No Aplica":dataglb[18]
        var agencia = (dataglb[19] == "" || dataglb[19] == null)?"No Aplica":dataglb[19]
        row.child(format_proc_fin(cedula, mail, phone, producto, tincidencia, stincidencia, area, trespuesta, creadop, aasignado, canal, redsocial, agencia)).show()
        tr.addClass('shown')
    }
});

/**************************************************************************Finalizado*/
$('#datatable-ajax-finalizado tbody').on('click', 'td.details-control', function(){
    var tr = $(this).closest('tr')
    ...
    if (row.child.isShown()){
        ...
        var redsocial = (dataglb[18] == "" || dataglb[18] == null)?"No Aplica":dataglb[18]
        var agencia = (dataglb[19] == "" || dataglb[19] == null)?"No Aplica":dataglb[19]
        row.child(format_finish(cedula, mail, phone, producto, tincidencia, stincidencia, area, trespuesta, creadop, aasignado, canal, redsocial, agencia)).show()
        tr.addClass('shown')
    }
});
...
function format(cedula, mail, phone, producto, tincidencia, stincidencia, area, trespuesta, creadop, aasignado, ccanal, rredsocial, aagencia, ccomentario){
...
function format_proc_fin(cedula, mail, phone, producto, tincidencia, stincidencia, area, trespuesta, creadop, aasignado, ccanal, rredsocial, aagencia){ //Cambio final 006
    return '<table class="table table-striped table-bordered">'+
        '<tr>'+
            '<td>Cédula:</td>'+
            '<td>' + cedula + '</td>'+
...
function format_finish(cedula, mail, phone, producto, tincidencia, stincidencia, area, trespuesta, creadop, aasignado, ccanal, rredsocial, aagencia){ //Cambio final 007
    return '<table class="table table-striped table-bordered">'+
        '<tr>'+
            '<td>Cédula:</td>'+
            '<td>' + cedula + '</td>'+
                        ...
                        setTimeout((function(){
                            ...
                            $.post('backend/udpedittickets.php',{
                                ...
                                if (parseData == "OK"){
                                    ...
                                    var body = "<html>Estimado Supervisor,<br></br><br></br>"
                                        body += "Se le notifica que el " + nticketid + ", ha sido modificado por el usuario " + aasignado + " con los siguientes datos:<br></br><br></br>"
                                    ...
                                    SendMailGlobal(correosupervisor, subject, body)
                                    /***********************************************************************************************************************/
...
$('#gestionTCK').on('shown.bs.modal', function(){
    $("#maxuploadcant").text("Cargar archivos o print (Max. " + maxuploadfile + " archivos - 4Mb)")
    upload_files()
    getTime()
...
$("#ticketestado").change(function(){
    ...
    if (estadotg == "EN PROCESO" || estadotg == "FINALIZADO"){
        ...
        $("#commentstg").removeAttr("hidden")
        $("#kt_gestion_ticket_submit").removeAttr("hidden")
        $("#ticketescalar").load("backend/getuserssuporq.php",{profileid: profileid})
        tUs = 1
    }
})
...
var validatorUpdateTicket = FormValidation.formValidation(
    form, {
        fields:{
            ticketestado:{
                validators:{
...
var validatorUpdateTicketEscalar = FormValidation.formValidation(
    form, {
        fields:{
            ticketestado:{
                validators:{
                        ...
                        $.post('backend/udptickets.php',{
                            ...
                            if (parseData.Result == "OK"){
                                ...
                                if (estadoticket == "FINALIZADO"){
                                    ...
                                    var urlencuesta = "http://186.46.61.28:54667/dist/authentication/flows/aside/quiz-form-ticket?TicketId=" + ticketidg //Producción
                                    ...
                                    /*var body = "<html>Estimado/a cliente,<br></br><br></br>"
                                        ...
                                        body += "Saludos,<br></br>"
                                        body += "Administrador del sistema</html>"*/
                                    var body = "<a href='" + urlencuesta + "'><img src='https://drive.google.com/uc?export=view&id=1j-MLd5C_8ID9faI4oCnrPhK5UZQ4YmgS' alt='ticket-finalizado' border='0'></a>"
                    ...
                    setTimeout(function (){
                        ...
                        $.post('backend/udptickets.php',{
                            ...
                            if (parseData.Result == "OK"){
                                ...
                                body += "Se le ha escalado el " + nticketid + ", por el usuario " + localStorage.getItem("ProfileName") + ", con el siguiente detalle:<br></br><br></br>"
                                body += "<strong>Producto: " + $("#tipoproducto").val() +"</strong><br></br>"
                                body += "<strong>Tipo incidencia: " + $("#tipoincidencia").val() +"</strong><br></br>"
                                body += "<strong>Sub-tipo incidencia: " + $("#subtipoincidencia").val() +"</strong><br></br><br></br><br></br>"
...
$('#datatable-ajax-finalizado tbody').on('click', '.btn-filesupload', function(){
    if (ticketidg == "Sin Registro"){
        $(".btn-filesupload").attr("disabled","disabled")
    }
    else {
...
$('#datatable-ajax-finalizado tbody').on('click', '.btn-time-ocup', function(){
    if (ticketidg == "Sin Registro"){
        $(".btn-time-ocup").attr("disabled","disabled")
    }
    else {
...
/*Dialogo Reapertura*/
alertify.reopendialog || alertify.dialog('reopendialog',function(){
    return {
        main:function(content){
            this.setContent(content)
        },
        ...
        init:function(){
            ...
            e.addEventListener("click",(function(n){
                ...
                i.validate().then((function(i){
                        ...
                        setTimeout((function(){
                            ...
                            $.post('backend/setreopenticket.php',{
                                ...
                                if (parseData.Result == "OK"){
                                    ...
                                    var urlReopenticket = "http://186.46.61.28:54667/dist/authentication/flows/aside/reopen-form-ticket?TicketId=" + ticketidg //Producción
                                    var subject = "Re-apertura " + nticketid
                                    var body = "<html>Estimado supervisor " + adminName +",<br></br><br></br>"
                                        body += "El asesor " + pname + " a solicitado la re-apertura de el " + nticketid + ", puede autorizarlo ingresando en el siguiente link: <br></br><br></br>"
                                        body += "<a href=" + urlReopenticket + ">Ir a autorizar</a><br></br><br></br><br></br>"
                                        body += "Saludos,<br></br>"
                                        body += "Administrador del sistema</html>"
                                    SendMailGlobal(adminMail, subject, body)
                                    /***********************************************************************************************************************/
                                    Swal.fire({
                                        text: "Se ha enviado su solicitud al supervisor, " + adminName,
                                        icon: "success",
                                        buttonsStyling: !1,
                                        confirmButtonText: "OK",
                                        customClass: {
                    ...
                    2e3)):console.log("Sin comentarios")
                }))
            }))
        }
    }
...
/*Funcion tiempo*/
function getTime(){ //Gestion actual tiempo
    seconds = 0
...
$("#search-ticket").click(function(){
    var pusertype = localStorage.getItem("ProfileUserType")
    if (pusertype == "ingresador" || pusertype == "orquestador" || pusertype == "administrador"){ //Cambio final 003
        window.location = "user-assigned-ticket"
    }
    else {
