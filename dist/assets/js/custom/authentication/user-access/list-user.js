    ...
    columnDefs: [
      {
        ...
        data: null,
        defaultContent:
          '<button type="button" class="btn btn-primary btn-sm btn-activate-deactivate-user" data-bs-toggle="tooltip" data-bs-placement="top" title="Editar usuario" name="btn-activate-deactivate-user"><i class="fa fa-users"></i></button>',
      },
      {
        title: "Eliminar",
        targets: 1,
        data: null,
        defaultContent:
          '<button type="button" class="btn btn-danger btn-sm btn-delete-user" data-bs-toggle="tooltip" data-bs-placement="top" title="Eliminar usuario" name="btn-delete-user"><i class="fa fa-times"></i></button>',
      },
...
$("#datatable-ajax-list-users tbody").on(
  "click",
  ".btn-activate-deactivate-user",
  function () {
    $("#kt_new_user_submit").removeAttr("disabled");
    ...
    if (data[11] == "agencia") {
      ...
      var selectList =
        "<select class='form-control form-control-lg form-select' name='personalasignado' id='personalasignado' autocomplete='off'>";
...
alertify.newuser ||
  alertify.dialog("newuser", function () {
    return {
...
var validatorOnlyUser = FormValidation.formValidation(form, {
  fields: {
    "nusuario-user": {
      validators: {
        notEmpty: {
...
var validatorAgenciaUser = FormValidation.formValidation(form, {
  fields: {
    "nusuario-user": {
      validators: {
        notEmpty: {
...
$("#area-usuario").change(function () {
  if (
    $("#perfil-usuario").val() == "orquestador" ||
    $("#perfil-usuario").val() == "ingresador"
  ) {
  ...
  if (
    $("#area-usuario").val() == "Cobranzas" ||
    $("#area-usuario").val() == "Riesgos" ||
    $("#area-usuario").val() == "Operaciones" ||
    $("#area-usuario").val() == "Marketing y Negocios" ||
    $("#area-usuario").val() == "Negocios" ||
    $("#area-usuario").val() == "Atención al Cliente"
  ) {
    $("#cargoglb").removeAttr("hidden");
    $("#cargo-usuario").load("backend/getcatalogocargos.php", {
      area: $("#area-usuario").val(),
    });
  } else {
    $("#cargoglb").attr("hidden", "hidden");
  }
